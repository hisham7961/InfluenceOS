...
describe('Content association — create/update through the real API', () => {
  ...
  beforeAll(async () => {
    ...
    influencerId = idOf(await app.inject({ method: 'POST', url: '/api/v1/influencers', headers: auth, payload: { displayName: `${tag} Sara` } }));
    otherInfluencerId = idOf(await app.inject({ method: 'POST', url: '/api/v1/influencers', headers: auth, payload: { displayName: `${tag} Ahmed` } }));
