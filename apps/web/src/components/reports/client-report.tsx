...
/**
 * The client report itself (P2.2): what was promised against what was
 * delivered, per creator and per post, in the report's own language
 * (independent of the reader's). Shown in the app and on a shared link
 * (P3.3); prints cleanly to PDF from the browser.
 */
export async function ClientReport({ report }: { report: CampaignReportDTO }) {
              ...
              value={
                report.sales.revenue.length
                  ? report.sales.revenue
                      .map((r) => formatCurrency(r.amount, r.currency))
                      .join(' · ')
              ...
              <Kpi
                ...
                sub={
                  report.sales.costPerOrder != null
                    ? t('costPerOrder', {
                        amount: formatCurrency(report.sales.costPerOrder, currency),
                      })
                    : null
                }
                percent={null}
                        ...
                        <Td end>
                          <LtrText>
                            {r.sales?.revenue.length
                              ? r.sales.revenue
                                  .map((m) => formatCurrency(m.amount, m.currency))
                                  .join(' · ')
                              : '—'}
                          </LtrText>
                        </Td>
              ...
              <div key={p.id} className="border-border overflow-hidden rounded-xl border">
                <div className="bg-surface-muted flex aspect-[4/5] items-center justify-center">
                  {p.thumbnailUrl ? (
                    ...
                    <PlatformIcon platform={p.platform} className="text-muted-foreground h-8 w-8" />
                  )}
                </div>
