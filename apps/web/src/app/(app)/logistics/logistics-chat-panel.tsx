...
function Thread() {
  return (
    <CommentThread
      ...
      emptyDescription="Coordinate with the logistics team here — couriers, delays, anything that isn't about one specific shipment."
      composerPlaceholder="Message the logistics team… use @ to mention someone"
    />
  );
}
    ...
    <>
      ...
      <div className={cn('hidden shrink-0 lg:block', open ? 'w-80' : 'w-auto')}>
        {open ? (
          <Card className="sticky top-4 flex max-h-[calc(100vh-6rem)] w-80 flex-col overflow-hidden">
            <div className="flex items-center justify-between border-b border-border px-4 py-3">
              <span className="flex items-center gap-1.5 text-sm font-semibold">
                <MessageSquare className="h-4 w-4 text-accent" /> Logistics Chat
              </span>
              <Button type="button" variant="ghost" size="icon-sm" title="Collapse chat" onClick={() => setOpen(false)}>
          ...
          <Button type="button" variant="outline" size="icon" title="Open chat" onClick={() => setOpen(true)} className="sticky top-4">
