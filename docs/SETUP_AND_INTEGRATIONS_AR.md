...
> **تنبيه مهم للسيرفر (Docker):** ملف `docker-compose.full.yml` يمرّر للحاويات
> قائمة محددة فقط من القيم (قاعدة البيانات، التخزين، `AUTH_SECRET`…). أي قيمة
> ثانية تكتبها في `.env` — مثل مفاتيح المنصات، `LOGIN_MAX_ATTEMPTS`،
> `MONITOR_CRON`، `MAX_UPLOAD_MB` — **لا تصل** للـ API ولا للـ worker إلا إذا
> أُضيفت لملف override. لمفاتيح المنصات استخدم شاشة الأدمن (القسم 4-ب) لأنها
> تشتغل مباشرة بلا أي تعديل على السيرفر.
...
| `MAX_UPLOAD_MB` | أقصى حجم رفع — على سيرفر Docker مثبّت حالياً على **50** ميغابايت في `docker-compose.full.yml` (القيمة في `.env` لا تُطبَّق)؛ محلياً الافتراضي 50 |
...
### 3.3 Instagram / Meta — الملف + إعجابات وتعليقات المنشورات (حساب أعمال)
يعمل عبر **Business Discovery** ويحتاج حساب انستغرام **Professional
(Business/Creator)**. يعطي للحسابات الاحترافية فقط: الاسم والصورة والنبذة وعدد
المتابعين، **بالإضافة إلى الإعجابات والتعليقات — ومشاهدات الريلز — لكل منشور**
من منشوراتها **الحديثة** (عبر طرف `media`).

> **اختر المسار الصحيح.** تحت منتج Instagram تعرض Meta مسارين:
> «API setup with **Instagram** login» و«API setup with **Facebook** login».
> و`business_discovery` موجودة في **مسار الفيسبوك فقط** (`graph.facebook.com`)؛
> أما مسار إنستغرام (`graph.instagram.com`) فلا تحوي هذه الحافة أصلاً، ولا
> يُصلحها رمز ولا إذن. كما أن Instagram Basic Display API أُوقفت في 2024-12-04،
> فأي شرح مبني عليها ميّت.

الأذونات: `instagram_basic` و`instagram_manage_insights` و`pages_read_engagement`،
مع `pages_show_list` لخطوة `/me/accounts` (و`ads_management` أو `ads_read` إن
مُنح دور الصفحة عبر Business Manager).

**الترتيب إلزامي:** بدّل الرمز القصير برمز طويل الأمد **قبل** استدعاء
`/me/accounts`. فرمز الصفحة المشتقّ من رمز قصير يرث الانتهاء القصير ويموت خلال
ساعة، بينما المشتقّ من رمز طويل الأمد لا ينتهي.

لا تلزم App Review ولا Business Verification ما دام التطبيق يخدم أصحاب الأدوار
عليه فقط — فالمؤثّرون المستعلَم عنهم لا يسجّلون دخولاً ولا يؤثّرون على مستوى
الوصول.

مقاييس المنشور تُطابَق برمز المنشور (shortcode) مقابل روابط (`permalink`) الحساب
المالك، فيلزم أن يكون للمؤثّر سجل `SocialAccount` لانستغرام — فاسم الحساب هو ما
يُبحث به (رابط منشور انستغرام لا يحمل اسم الحساب). المنشور الأقدم من نافذة
المنشورات الحديثة يُرجِع `NOT_FOUND` ويُترك ما أُدخل يدوياً كما هو.
...
- **لا يعطي شيئاً للحسابات الشخصية** — غير قابلة للاكتشاف، فتبقى يدوية بالكامل.
- **المشاهدات للريلز فقط** — `view_count` يشمل المدفوع والعضوي معاً، بينما
  `total_views_count` غير متاح عبر هذه الحافة نصّاً.
- **`like_count` يختفي** إن أخفى صاحب المنشور عدد الإعجابات: تحذفه Meta تحت
  field expansion بدل أن تُرجِع خطأً، فيُقرأ null.
- لا توجد نقطة نهاية تحوّل رابط منشور إلى معرّف وسائط، ولا يمكن استدعاء المعرّف
  العائد من Business Discovery مباشرة، فكل الحقول تأتي عبر field expansion في
  الاستدعاء الواحد.
- تيك توك يحتاج تفويض المؤثر نفسه (Creator-OAuth، القسم 5).
...
**أ) عبر ملف البيئة (`.env`)** — تشتغل في بيئة التطوير فقط. على سيرفر Docker
لا تصل هذه القيم للحاويات (انظر التنبيه أعلى الملف)، فاستخدم الطريقة (ب).
...
- من الواجهة: **Settings → Integrations → "Provider API keys"** (تظهر للأدمن
  ...
  البيئة فوراً دون إعادة تشغيل (الـ API فوراً، والـ worker في دورة المراقبة
  التالية — خلال 30 دقيقة كحد أقصى).
